...
- **2026-07-14:** `wendigo` merged the PR via the merge queue into `trinodb:master` — merge commit [`56d8d8f`](https://github.com/trinodb/trino/commit/56d8d8f), 99 checks passed. Added to milestone 483. `martint` (Trino co-founder) commented: *"Thanks for your contribution, @minnocent12! This will be in the next release. Try it live here: https://trysql.io/3-YGq6IqjSUO"* — live demo confirms `title_case('the HOME depot')` → `The Home Depot`, `title_case('österreich')` → `Österreich`. **PR MERGED. Contribution #1 complete.**
