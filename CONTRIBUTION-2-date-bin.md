...
- **2026-07-29:** `findepi` (Trino maintainer) reviewed. Two inline comments on `datetime.md`: (1) positive acknowledgment that the signature follows PostgreSQL's `date_bin` spec; (2) syntax question — linked to issue #20428 asking "What is the way to do this kind of binning in other SQL engines (e.g. current Trino, Snowflake, BigQuery, Oracle, SQL Server, MySQL)?" Responded on issue #20428 with a cross-engine survey showing PostgreSQL, Snowflake, and Spark SQL all use the identical `date_bin(stride, source, origin)` signature, while BigQuery and DuckDB use different names for the same semantics, and Oracle/SQL Server/MySQL have no native equivalent. This demonstrates cross-engine convergence on the proposed signature.

**Status:** Iterating — awaiting `findepi` response to cross-engine survey on issue #20428
